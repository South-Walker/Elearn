...
namespace Elearn.Controllers
{
    ...
    public class ISEServerAgent:IDisposable
    {
        ...
        public void AudioWrite(List<byte> bytes)
        {
            IntPtr h = UnmanagedManager.UnmanagedReader(bytes);
        ...
        public void AudioWrite(byte[] bytes)
        {
            IntPtr h = UnmanagedManager.UnmanagedReader(bytes);
    ...
    public class UnmanagedManager
    {
        ...
        public static IntPtr UnmanagedReader(List<byte>bs)
        {
            IntPtr h = Marshal.AllocHGlobal(bs.Count);
            for (int i = 0; i < bs.Count; i++)
            {
                Marshal.WriteByte(h, i, bs[i]);
            }
            return h;
        }
        public static IntPtr UnmanagedReader(byte[]bs)
        {
            IntPtr h = Marshal.AllocHGlobal(bs.Length);
